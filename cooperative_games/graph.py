...
class CoopGraph:
    ...
    def update_node_weigths(self, nodes_weights: torch.Tensor):
        self.nodes_weights = nodes_weights.clone().detach()
    ...
    def visualize_graph_structure(self):
        ...
        for id in range(self.num_nodes):
            label_str = f"Node {id} \n"
            label_str += "Pop: " + str(tuple(np.round(self.nodes_feats[id].numpy(), num_digits_round))) + "\n"
            label_str += "Fit: " + str(tuple(np.round(self.nodes_strategy_fit[id].numpy(), num_digits_round)))
            title_str = f"Payoff matrix \n {np.matrix(self.payoff_matrices[id].numpy())}"
    ...
    def describe_egtgraph(self, verbose=False):
        if verbose:
            ...
            print(f"Nodes weights: {self.nodes_weights.numpy()}")
